...
async function main() {
  ...
  const giras = await leerDatosGiras();
...
main().then(() => {
  ...
  // tengo que agregar a la matriz los valores de las giras.
  // por cada elemento de la gira busco el nombre en la matriz y asigno valores
  // for (const jugador of giras) {
  //   const textoBuscado = jugador.Jugador;
  //   console.log (`texto Buscado : ${textoBuscado}`)
  //   for (let i = 0; i < 12; i++) {
  //     if (matrizValores[i][1] == textoBuscado) {
  //       console.log (`jugador ${textoBuscado}, sierra : ${jugador.Sierra}`)
  //       matrizValores[i][5] += jugador.Sierra;
  ...
  console.table("matriz valores", matrizValores);
  ...
  // agrega resultados de la gira.

  // for (let i = 0; i < 12; i++) {
  //   for (let j = 3; j < 11; j++) {
  //     matrizValores[i][2] += matrizValores[i][j];
  //     console.log(matrizValores[i][j]);
  //   }
  // }
  ...
  for (let i = 0; i < matrizValores.length; i++) {
    ...
    for (let j = 0; j < matrizValores[i].length; j++) {
      ...
      if (j === 3) {
        ...
        enlace.href = `transation-detail.html?player=${encodeURIComponent(
          matrizValores[i][1]
        )}`;
...
async function leerDatosGiras() {
  try {
    ...
    if (results.ok) {
      ...
      console.log("leyo en funcion giras", resultadosGira);
